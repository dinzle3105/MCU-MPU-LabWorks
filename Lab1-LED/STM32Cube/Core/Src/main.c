...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Traffic Light Simulation (Active-Low)
  ******************************************************************************
  */
...
void ex2(void) {
    // Phase 1: RED Light ON (5 Seconds)
    ...
    // Phase 2: GREEN Light ON (3 Seconds)
    HAL_GPIO_WritePin(GPIOA, LED_RED_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOA, LED_YELLOW_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOA, LED_GREEN_Pin, GPIO_PIN_RESET);
    HAL_Delay(3000);

    // Phase 3: YELLOW Light ON (2 Seconds)
    HAL_GPIO_WritePin(GPIOA, LED_RED_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOA, LED_YELLOW_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOA, LED_GREEN_Pin, GPIO_PIN_SET);
    HAL_Delay(2000);
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // Initial State: Turn all LEDs OFF (Active-Low: drive HIGH)
...
static void MX_GPIO_Init(void)
{
  ...
  /* Configure initial Output Level to SET (OFF for Active-Low) */
  HAL_GPIO_WritePin(GPIOA, LED_RED_Pin|LED_YELLOW_Pin|LED_GREEN_Pin, GPIO_PIN_SET);
